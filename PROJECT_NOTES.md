...
**Local path:** ~/Development/hebrew-text-visualizer
...
src/main/java/com/hebrewproject/
  model/        Verse, Word, Root, RootOccurrence
...
- Repo initialized, `.gitignore` excludes `target/` and `data/` (local H2 db
  regenerates fresh on ingestion, shouldn't be committed)
